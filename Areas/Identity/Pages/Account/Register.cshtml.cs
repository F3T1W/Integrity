...
namespace Integrity.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private IntegrityUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<IntegrityUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(IntegrityUser)}'. " +
                    $"Ensure that '{nameof(IntegrityUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<IntegrityUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<IntegrityUser>)_userStore;
        }
    }
}
